...
function isObject(o: object) { return (!Array.isArray(o) && o instanceof Object && Try(() => JSON.stringify(o)[0] === '{', false)) }
function isArray(o: object) { return (Array.isArray(o)) }
...
function parseNull(data: Record<string, any> | any[]) {
    try {
...
function trimObject<T extends Record<string, any>>(_object: T, option?: { with_null: boolean }): {
...
function distinctArray<T extends ({ [key: string]: any }[])>(array: T, key: keyof T[number]): T
function distinctArray<T extends any[]>(array: T): T
function distinctArray<T extends ({ [key: string]: any }[]) | any[]>(array: T, key?: T extends ({ [key: string]: any }[]) ? keyof T[number] : undefined): T {
...
function blackListKey<T extends Record<string, any> | Record<string, any>[]>(object: T, black_list: string[]): T {
    try {
...
function mapper(obj: { [key: string]: any }[], pattern: { [key: string]: any }, parrent_col?: string, parrent_col_id?: number, key_idx: number = 0): any {
    try {
